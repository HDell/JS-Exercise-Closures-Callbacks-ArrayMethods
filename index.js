...
function firstNamesAllCaps(runners) {
  const runnerNameCapped = runners.map(runner => runner.first_name.toUpperCase());
  return runnerNameCapped;
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  const runnersWithSize = runners.filter(runner => runner.shirt_size === tShirtSize);
...
function tallyUpDonations(runners) {
  const talliedDonations = runners.reduce((total, runner) => {
